...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (itr != _factions.end())
    {
        ...
        if (incremental)
        {
            ...
            standing = int32(floor((float)standing * (_player->GetPersonalRate() ? _player->GetPersonalRate() : sWorld->getRate(RATE_REPUTATION_GAIN)) + 0.5f));
            
            if(_player->GetSession()->IsPremium())
               standing *= sWorld->getRate(RATE_REPUTATION_GAIN_BOOST_PREMIUM);

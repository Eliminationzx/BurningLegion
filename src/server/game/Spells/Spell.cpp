...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_FRONT_LEAP:
        {
            ...
            {
                float distance = effect->CalcRadius(m_caster);
                Map* map = m_caster->GetMap();
                uint32 mapid = m_caster->GetMapId();
                PhaseShift phaseshift = m_caster->GetPhaseShift();
                ...
                ground = map->GetHeight(phaseshift, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
                ...
                if (!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) || (pos.GetPositionZ() - ground < 25.0f))
                {
                    ...
                    for (; j < (numChecks + 1); j++)
                    {
                        ...
                        tstZ = map->GetHeight(phaseshift, tstX, tstY, prevZ + maxtravelDistZ, true);
                        ground = tstZ;

                        if (!map->IsInWater(phaseshift, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ()))
                        {
                            if (map->IsInWater(phaseshift, tstX, tstY, tstZ))
                            {
                        ...
                        else if (map->IsInWater(phaseshift, tstX, tstY, tstZ))
                        {
                            prevZ = pos.GetPositionZ();
                        ...
                        if (!map->IsInWater(phaseshift, tstX, tstY, tstZ) && tstZ != beforewaterz)  // second safety check z for blink way if on the ground
                        {
                            // highest available point
                            tstZ1 = map->GetHeight(phaseshift, tstX, tstY, prevZ + maxtravelDistZ, true, 25.0f);
                            // upper or floor
                            tstZ2 = map->GetHeight(phaseshift, tstX, tstY, prevZ, true, 25.0f);
                            //lower than floor
                            tstZ3 = map->GetHeight(phaseshift, tstX, tstY, prevZ - maxtravelDistZ / 2, true, 25.0f);
                        ...
                        bool dcol = m_caster->GetMap()->getObjectHitPos(phaseshift, prevX, prevY, prevZ + 0.5f, tstX, tstY, tstZ + 0.5f, tstX, tstY, tstZ, -0.5f);

                        // collision occured
                        if (col || dcol || (overdistance > 0.0f && !map->IsInWater(phaseshift, tstX, tstY, ground)) || (fabs(prevZ - tstZ) > maxtravelDistZ && (tstZ > prevZ)))
                        {
                            ...
                            // highest available point
                            destz1 = map->GetHeight(phaseshift, destx, desty, prevZ + maxtravelDistZ, true, 25.0f);
                            // upper or floor
                            destz2 = map->GetHeight(phaseshift, destx, desty, prevZ, true, 25.0f);
                            //lower than floor
                            destz3 = map->GetHeight(phaseshift, destx, desty, prevZ - maxtravelDistZ / 2, true, 25.0f);
                ...
                else
                {
                    ...
                    // check dynamic collision
                    bool dcol = m_caster->GetMap()->getObjectHitPos(phaseshift, pos.GetPositionX(), pos.GetPositionY(), z + 0.5f, destx, desty, z + 0.5f, destx, desty, z, -0.5f);
